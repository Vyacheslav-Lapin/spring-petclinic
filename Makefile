...
uninit-full: clear uninit
	rm -rf .idea spring-petclinic.iml .git

reboot-full: uninit-full init
	echo "\n/.idea/\n/spring-petclinic.iml\n/out/\n/classes/\n" >> .git/info/exclude
...
build:
	./mvnw verify
	chmod +x ./target/spring-petclinic-0.0.1-SNAPSHOT.jar

run:
	./mvnw spring-boot:start -Dspring.profiles.active=local
#	./target/spring-petclinic-0.0.1-SNAPSHOT.jar
#	java -jar --enable-preview ./target/spring-petclinic-0.0.1-SNAPSHOT.jar
...
git-fork-init: init
	git remote add upstream git://github.com/Vyacheslav-Lapin/spring-petclinic.git
	git fetch upstream
